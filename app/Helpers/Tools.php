...
class Tools
{
	...
	foreach($_POST as $key => $value){
		...
		if( strstr($key, 'id-') ){
			...
			$obj->updateImage( $_POST[$key],$_POST['alt-'.$file_num], $ext,$_POST['alignment-'.$file_num],$_POST['line-'.$file_num],$_POST['line1-'.$file_num],$_POST['line2-'.$file_num],$_POST['btntext-'.$file_num] );
	...
	if(in_array($key, $uploadedArray)){ continue; }
		...
		if($_FILES[$key]['size'] > 0){
        	...
        	$alignment_key = str_replace('file', 'alignment', $key);
            $alignment = $_POST[$alignment_key];

            $line_key = str_replace('file', 'line', $key);
            $line = $_POST[$line_key];
			...
			$image_id = $obj->addImage( $id, $alt, $ext,$alignment, $line, $line1,$line2,$btntext );
