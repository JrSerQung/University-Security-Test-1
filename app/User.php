...
class User extends ObjectModel
{
    ...
    protected $fillable = ['email', 'password', 'first_name', 'last_name', 'address_1', 'address_2', 'phone', 'town', 'postcode', 'country', 'contacts','company_name','trading_name','company_director','vat_number','company_registration_number','trade_platforms','credit_reference','credit_user','trade_type','verified' ];
...
/*
    ...
    public function login($role, $basketItemCount = false)
    {
        ...
        $result = $this->execute(' SELECT * FROM users WHERE email = ? AND password = ? AND user_role_id = ? AND deleted_at IS NULL AND verified = 1', [ $_POST['email'], Tools::passwordHash($_POST['password']), $role ]);

        if( !count($result) ){

            return isset($_POST['redirect']) ? redirect( $_POST['redirect'], 'Your login credentials were incorrect or the acccount has not been approved yet!', 'e' ) : redirect( 'login.php', 'Your login credentials were incorrect or the acccount has not been approved yet!', 'e' );
    ...
    {
        $this->updateRow($this->table, ['verified' => 1], 'id = :id ' , [ 'id' => $id ] );
        $this->updateRow($this->table, ['trade_type' => $_POST["class"]], 'id = :id ' , [ 'id' => $id ] );
        $this->updateRow($this->table, ['credit_user' => $_POST["credit_user"]], 'id = :id ' , [ 'id' => $id ] );
    ...
    public function updateUser($id)
    {
        $this->updateRow($this->table, ['trade_type' => $_POST["class"]], 'id = :id ' , [ 'id' => $id ] );
        $this->updateRow($this->table, ['credit_user' => $_POST["credit_user"]], 'id = :id ' , [ 'id' => $id ] );
        return redirect(DOMAIN. "/console/account?page=users");
    }

    public function declineVerify($id)
    {
    ...
    {
        $this->updateRow($this->table, ['deleted_at' => null], 'id = :id  ', [ 'id' => $id ] );
        return redirect(DOMAIN. "/console/account?page=users");
    ...
    public function getPendingVerify()
    {
        return $this->execute('SELECT * FROM users WHERE user_role_id = 1 AND deleted_at IS NULL AND verified = 0', [1] );
    }
    public function getPendingVerifyRverse()
    {
        return $this->execute('SELECT * FROM users WHERE user_role_id = 1 AND deleted_at IS NULL AND verified = 1', [1] );
    }
    public function getDeleteds()
    {
        return $this->execute('SELECT * FROM users WHERE user_role_id = 1 AND deleted_at IS NOT NULL', [] );
    }
