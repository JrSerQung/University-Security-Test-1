...
class Product extends ObjectModel{
    ...
    protected $fillable = ['title', 'seo_url', 'category_id', 'sub_category_id','brand', 'price', 'special_offer_price', 'description', 'attributes', 'best_seller', 'product_code', 'weight', 'sku', 'sale_item', 'width', 'height', 'depth', 'capacity', 'dimensions_extra', 'dimensions_extra_2', 'product_status', 'deal_day', 'deal_week', 'delivery_options','qty_available'];
    protected $rules = [
					'title' => 'required',
	...
	$query = $this->execute('SELECT *, products.title AS product_title, categories.title AS category_title, products.seo_url AS product_seo_url, 
    ...
    public function getAllBrandsSub($category_id)
    {

        $query = $this->execute('SELECT brand FROM products WHERE deleted_at IS NULL AND brand != "" AND sub_category_id LIKE ? GROUP BY brand', ["%". $category_id ."%"] );
    ...
    public function getMinPriceSub($category_id)
    {

        $query = $this->execute('SELECT price,special_offer_price FROM products WHERE deleted_at IS NULL AND sub_category_id LIKE ?', ["%". $category_id ."%"] );
        foreach ($query as $item){
            if($item->special_offer_price != "" and $item->special_offer_price != null){
                $item->price = $item->special_offer_price;
            }
	...
	return $this->execute('SELECT * FROM products WHERE deleted_at IS NULL ORDER BY id DESC LIMIT 12  ', [] );
    ...
    public function getAllByCategory($category_id)
    {
        ...
        if(count($options) > 0){
            ...
            $result = "SELECT * FROM products WHERE category_id LIKE '%". $category_id ."%' AND deleted_at IS NULL AND price <= '".$maxPrice ."'";
        }
      
	    return $this->execute($result, [] );

    }
    public function getAllByCategoryLimit($category_id,$limit)
    {
        $maxPrice = 99999999;
        if(isset($_GET["max_price"])){
            $maxPrice = $_GET["max_price"];
        ...
        if(count($options) > 0){
            ...
            $result = "SELECT * FROM products WHERE category_id LIKE '%". $category_id ."%' AND deleted_at IS NULL AND ".implode('', $searchTermBits) . " LIMIT " . $limit;
            $result = substr($result,0,-5);
            ...
            $result = "SELECT * FROM products WHERE category_id LIKE '%". $category_id ."%' AND deleted_at IS NULL AND price <= '".$maxPrice ."' LIMIT ". $limit;
        }
    ...
    public function getAllBySubCategory($category_id)
    {
        $maxPrice = 99999999;
        if(isset($_GET["max_price"])){
            $maxPrice = $_GET["max_price"];
        ...
        if(count($options) > 0){
            ...
            foreach ($options as $term) {
                ...
                if (!empty($term)) {
                    $searchTermBits[] = " brand LIKE '%$term%' OR sub_category_id LIKE '%". $category_id."%' AND deleted_at IS NULL AND brand LIKE '%$term%' AND price <= '".$maxPrice ."' AND ";
                }
            }

            $result = "SELECT * FROM products WHERE sub_category_id LIKE '%". $category_id ."%' AND deleted_at IS NULL AND ".implode('', $searchTermBits);
            $result = substr($result,0,-5);
            ...
            $result = "SELECT * FROM products WHERE sub_category_id LIKE '%". $category_id ."%' AND deleted_at IS NULL AND price <= '".$maxPrice ."'";
        }
...

    {
        ...
        $options = [];
        $brands = $this->getAllBrandsAllCategories();
        $bX = 0;
        foreach ($brands as $brand) {
            $bX++;
            if(isset($_GET["brand". $bX])){
        ...
        if(count($options) > 0){
            ...
            foreach ($options as $term) {
                ...
                if (!empty($term)) {
                    $searchTermBits[] = " brand LIKE '%$term%' OR category_id LIKE '%%' AND deleted_at IS NULL AND brand LIKE '%$term%' AND price <= '".$maxPrice ."' AND ";
                }
            }

            $result = "SELECT * FROM products WHERE category_id LIKE '%%' AND deleted_at IS NULL AND ".implode('', $searchTermBits);
            $result = substr($result,0,-5);
            ...
            $result = "SELECT * FROM products WHERE category_id LIKE '%%' AND deleted_at IS NULL AND price <= '".$maxPrice ."'";
        }
